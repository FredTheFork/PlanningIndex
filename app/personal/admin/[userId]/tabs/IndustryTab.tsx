...
import {
  ...
  RefreshCw, Briefcase, ChevronDown, ChevronUp, Save, Copy, Package, Zap, Layers
} from 'lucide-react';
...
export default function IndustryTab({ userId, data, refreshData }: IndustryTabProps) {
  ...
  const handleGenerateAllBriefs = async () => {
    ...
    const confirmMsg = `Generate briefs for all ${industryServices.length} industry packs? This will generate or regenerate briefs for each service.`;
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-inter font-semibold text-gray-900 text-sm">
                  Industry Tier ({industryServices.length} packs)
                </h3>
                <p className="font-inter text-xs text-gray-500">
                  Generate briefs for all industry services at once
                </p>
              </div>
            </div>
            <button
...
function IndustryServiceSection({
  ...
  const briefCompleted = latestBrief?.status === 'completed';
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              <span className={`font-inter text-xs font-medium ${
                briefCompleted ? 'text-green-700' : latestBrief?.status === 'generating' ? 'text-blue-600' : 'text-gray-500'
              }`}>
                Brief: {briefCompleted ? 'Generated' : latestBrief?.status === 'generating' ? 'Generating...' : 'Not generated'}
              </span>
            </div>
          </div>
        </div>
